...
- 실행 중인 서버 확인: `POST /api/pub/version` → `{version, cmsVersion, buildTime}`
  또는 관리자 화면 사이드바 하단(`v0.6.10 · CMS 1.2.9` 형태로 표시).
...
## 0.6.10 (CMS 1.2.9) — 미디어 라이브러리 흡수

CMS 1.2.9를 흡수했다. 업로드된 파일을 **볼 방법이 없었고**, 파일은 "글에 붙는 순간"에만 존재할 수
있었다 — 미리 올려두면 매핑 없는 업로드라 GC가 24시간 뒤 지웠다.

라이브러리 소속은 `file_ref`에 **`map_key=0, file_type='LIBRARY'`** 매핑 한 줄로 표시한다
(스키마 변경 없음). 그 매핑이 곧 존재 근거라 빠지면 **다음 새벽에 조용히 사라진다.**
`deactivateFilesByOwner`는 LIBRARY 매핑 있는 파일을 건너뛴다 — 안 그러면 라이브러리 이미지를 쓴
글 하나를 지웠을 때 원본까지 죽어 다른 글이 같이 깨진다.

- `/adm/file` 화면: 썸네일 그리드/목록 토글, 확장자·사용여부·라이브러리·등록기간·파일명 필터,
  상세에 사용처 목록, 담기/빼기·다운로드·삭제·고아 정리 수동 실행.
- `MediaPickerModal`을 `ImageUpload`에 붙였다 → 배너·팝업·로고에서 "라이브러리에서 선택"이 생긴다.
- `SearchBar`에 `daterange` 타입 추가, `file_ref(file_id)` 인덱스 추가.
- ⚠ 썸네일은 `<img src="/api/pub/image/{id}">`로 못 띄운다 — 라이브러리에만 담긴 파일은 공개 연결이
  없어 익명 요청이 403이고 `<img>`는 Authorization 헤더를 못 싣는다. `AuthImage`가 blob으로 받아 띄운다.

### ⚠ CMS와 다르게 옮긴 곳 (이 서비스 사정)

1. **프로필 사진을 '사용 중'으로 센다.** 이 저장소는 프로필만 `file_ref`를 안 거치고
   `member.profile_file_id`로 **직접 참조**한다(문자열 PK라 file_ref에 못 넣는 예외).
   CMS 원문대로 `file_ref`만 세면 **회원 프로필 사진이 전부 '미사용'으로 보이고, 관리자가 지우면
   그 회원 프로필이 깨진다.** 목록 카운트·사용여부 필터·사용처 목록 세 곳 모두에 넣었고,
   사용처에는 `PROFILE — {닉네임}`으로 뜬다. 조회가 잦아져 `ix_member_profile_file` 인덱스도 뒀다.
2. **메뉴 `menu_id`를 박지 않고 시퀀스에 맡긴다.** 이 저장소는 CMS와 menu_id 대역이 달라서
   (고유 메뉴가 더 있다) CMS의 번호를 그대로 옮기면 기존 메뉴와 충돌한다.
3. 용도 라벨에 `HOBBY`(취미 대표이미지)·`HOBBY_EDITOR`(취미 본문이미지)를 추가했다.

### 같이 고친 것 — 접속이력에 영문 코드가 그대로 나오던 문제

`END_REASON_LABEL`에 이 저장소 고유 사유 `WITHDRAW`(회원 탈퇴)·`SUSPEND`(관리자 계정정지)가
빠져 있었다 — 상수는 8개인데 라벨은 6개였다. 없는 코드는 에러가 아니라 `?? r.endReason` 폴백으로
**영문 코드가 조용히 노출된다.** 주석이 가리키던 `LoginSessionService.END_*`도 실재하지 않는
경로라 `SessionEndReason`으로 바로잡았다.

### 테스트

`MediaLibraryTest`(8) — CMS의 7개 + **프로필 사진이 '사용 중'으로 잡히는지** 1개.
