 ...
 * 조회 {path}: ''=차단 목록, Ids=차단한 ID 목록, Check=특정 회원 차단 여부.
 */
@RestController
...
public class BlockController {
    ...
    @PostMapping("/selectBlockList{path}.do")
    public ApiResponse<?> selectList(@PathVariable String path, @RequestBody BlockVO vo) {
        if ("Ids".equals(path)) {
