 ...
 * DB_ENC_KEY 미설정 시 BaseVO가 dev 기본키('psjyyb')로 폴백해 t_user PII를 약한 키로 암/복호화하는 사고를 막는다.
 */
@Slf4j
@Component
@Profile("prod")
public class ProdEnvGuard {
    ...
    @PostConstruct
    void check() {
        if (isBlank(System.getenv("DB_ENC_KEY"))) {
            throw new IllegalStateException(
                    "운영 환경변수 DB_ENC_KEY가 설정되지 않았습니다. (t_user PII 암호화 키 — 반드시 주입할 것)");
        }
