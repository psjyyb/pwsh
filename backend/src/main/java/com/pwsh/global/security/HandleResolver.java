...
/**
 * 공개 식별자(handle) ↔ 로그인 ID(user_id) 변환.
...
public class HandleResolver {
    ...
    public String toUserId(String handle) {
        if (handle == null || handle.isBlank()) {
            throw new BusinessException(ErrorCode.INVALID_INPUT, "대상 회원이 없습니다.");
        }
        UserVO p = new UserVO();
        p.setHandle(handle);
        UserVO u = commonDAO.selectOne("userDAO.selectByHandle", p);
    ...
    public String toHandle(String userId) {
        if (userId == null || userId.isBlank()) {
            return null;
        }
        UserVO p = new UserVO();
        p.setUserId(userId);
        UserVO u = commonDAO.selectOne("userDAO.selectByUserId", p);
