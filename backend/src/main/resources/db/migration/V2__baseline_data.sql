-- =====================================================================
-- V2 — baseline (기초데이터)
-- ★ 적용 뒤에는 고치지 않는다. 시드가 늘면 새 마이그레이션으로 추가한다.
-- =====================================================================
-- =====================================================================
-- 기본(seed) 데이터
...
-- ★ 아래 본문은 이 서비스가 실제로 수집·처리하는 항목에 맞춰 작성한 초안이다.
--   서비스 오픈 전에 운영자가 검토·확정해야 한다(상호·연락처·보유기간 등은 실제 값으로 교체).
--   개정 시에는 이 행을 수정하지 말고 use_yn='N' 처리 후 새 행을 등록한다 → member_policy 의 과거 동의 이력이 보존된다.
INSERT INTO policy (policy_id, title, content, type_cd, req_yn, sort_no, use_yn, reg_id, upd_id, reg_dt, upd_dt, reg_ip, upd_ip) VALUES
(1, '이용약관',
 '<h3>제1조 (목적)</h3><p>이 약관은 취만사(이하 "서비스")가 제공하는 취미 커뮤니티 서비스의 이용 조건과 절차, 회원과 서비스의 권리·의무를 정하는 것을 목적으로 합니다.</p>' ||
 '<h3>제2조 (회원가입)</h3><p>회원은 아이디·닉네임·비밀번호·이메일을 제공하고 이 약관 및 개인정보처리방침에 동의함으로써 가입합니다. 이메일 인증을 완료해야 가입이 성립합니다.</p>' ||
 '<h3>제3조 (서비스의 내용)</h3><p>서비스는 취미별 게시판, 모임 모집·참여 신청, 모임 참여자 간 대화, 쪽지, 후기 등의 기능을 제공합니다. 서비스의 구성은 운영상 필요에 따라 변경될 수 있습니다.</p>' ||
 '<h3>제4조 (회원의 의무)</h3><ul><li>타인의 정보를 도용하거나 허위 정보를 등록하지 않습니다.</li><li>욕설·차별·괴롭힘·광고·불법정보 등 타인에게 피해를 주는 게시물을 등록하지 않습니다.</li><li>모임에 참여할 때 상대방의 안전과 사생활을 존중하며, 알게 된 개인정보를 목적 외로 사용하지 않습니다.</li><li>계정을 타인에게 양도·대여하지 않습니다.</li></ul>' ||
 '<h3>제5조 (게시물의 관리)</h3><p>회원이 등록한 게시물의 권리는 작성자에게 있습니다. 다만 신고된 게시물이 제4조를 위반한 경우 서비스는 해당 게시물을 숨김 처리하거나 계정 이용을 제한할 수 있습니다.</p>' ||
 '<h3>제6조 (모임에 대한 책임)</h3><p>모임의 개설·진행·참여는 회원의 자율적 의사에 따릅니다. 서비스는 모임을 중개하는 공간을 제공할 뿐이며, 오프라인 모임에서 발생한 사고·분쟁에 대해 당사자를 대신해 책임지지 않습니다. 회원은 안전에 유의하여 참여해야 합니다.</p>' ||
 '<h3>제7조 (이용 제한 및 탈퇴)</h3><p>회원은 언제든지 마이페이지에서 탈퇴할 수 있으며, 탈퇴 시 계정은 비활성화됩니다. 서비스는 약관 위반이 확인된 회원의 이용을 제한할 수 있습니다.</p>' ||
 '<h3>제8조 (약관의 변경)</h3><p>약관이 변경되는 경우 서비스 내 공지사항을 통해 사전에 알립니다. 변경된 약관에 동의하지 않는 회원은 탈퇴할 수 있습니다.</p>' ||
 '<p style="color:#888">시행일: 서비스 오픈일 · 문의: 고객센터 1:1 문의</p>',
 'POLICY03', 'Y', 1, 'Y', 'system', 'system', NOW(), NOW(), '127.0.0.1', '127.0.0.1'),
(2, '개인정보처리방침',
 '<h3>1. 수집하는 개인정보 항목</h3><p><b>가입 시 필수</b>: 아이디, 닉네임, 비밀번호(단방향 암호화 저장), 이메일<br/><b>회원이 직접 입력하는 선택 항목</b>: 이름, 연락처, 생년월일, 성별, 프로필 사진<br/><b>서비스 이용 과정에서 자동 수집</b>: 접속 IP, 접속 일시, 이용 기록(로그인·등록·수정·삭제), 브라우저 정보</p>' ||
 '<h3>2. 수집·이용 목적</h3><ul><li>회원 식별 및 로그인, 계정 보안(비정상 접근 차단·비밀번호 만료 관리)</li><li>게시글·모집·쪽지 등 서비스 제공 및 작성자 표시(닉네임)</li><li>모임 참여 신청·수락 등 회원 간 연결</li><li>신고 처리, 부정 이용 방지, 서비스 개선을 위한 통계</li><li>이메일 인증 및 서비스 안내 메일 발송</li></ul>' ||
 '<h3>3. 보유 및 이용 기간</h3><p>회원 탈퇴 시 계정을 비활성화하고, 관계 법령상 보존 의무가 없는 개인정보는 지체 없이 파기합니다. 다만 분쟁 대응·부정 이용 방지를 위해 접속 기록은 관련 법령이 정한 기간 동안 보관할 수 있습니다.</p>' ||
 '<h3>4. 제3자 제공 및 처리 위탁</h3><p>회원의 개인정보를 제3자에게 제공하지 않습니다. 이메일 발송을 위해 메일 발송 서비스를 이용하며, 이 경우 발송에 필요한 최소한의 정보만 처리됩니다.</p>' ||
 '<h3>5. 개인정보의 보호 조치</h3><p>비밀번호는 복호화가 불가능한 방식으로 저장하고, 이름·연락처·이메일·생년월일 등은 암호화하여 보관합니다. 관리자 기능은 권한이 부여된 계정만 접근할 수 있으며 접근 이력을 기록합니다.</p>' ||
 '<h3>6. 회원의 권리</h3><p>회원은 언제든지 자신의 개인정보를 조회·수정할 수 있고, 탈퇴를 통해 개인정보의 이용 중지를 요청할 수 있습니다. 마이페이지 또는 1:1 문의를 통해 요청할 수 있습니다.</p>' ||
 '<h3>7. 공개 범위 안내</h3><p>다른 회원에게는 로그인 아이디가 아니라 닉네임과 공개 식별자만 표시됩니다. 게시판과 모집 글은 비로그인 방문자에게도 공개될 수 있습니다.</p>' ||
 '<p style="color:#888">시행일: 서비스 오픈일 · 개인정보 문의: 고객센터 1:1 문의</p>',
 'POLICY02', 'Y', 1, 'Y', 'system', 'system', NOW(), NOW(), '127.0.0.1', '127.0.0.1');
