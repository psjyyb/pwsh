-- =====================================================================
-- V1 — baseline (스키마)  ※ CMS 테이블 + pwsh 고유 테이블이 모두 들어 있다
-- ★ 이 파일이 스키마의 단일 출처다. 테이블을 바꾸려면 새 마이그레이션을 추가하고
--    이 파일은 손대지 않는다 — 이미 적용된 마이그레이션을 고치면 체크섬 불일치로 기동이 막힌다.
-- 번호 규칙: CMS에서 가져오는 것은 V3~V999, pwsh 고유 기능은 V1001~ 를 쓴다(충돌 방지).
--   ⚠ CMS의 V1·V2(베이스라인)는 가져오지 않는다 — 이 파일이 이미 그 시점을 포함한다.
-- =====================================================================
-- =====================================================================
...
CREATE TABLE config (
    ...
    maint_yn           VARCHAR(1)   DEFAULT 'N',
    maint_message      VARCHAR(500),
    title              VARCHAR(255),
...
COMMENT ON COLUMN config.maint_yn IS '점검(유지보수) 모드 여부(Y/N) — Y면 관리자 외 모든 요청 503';
COMMENT ON COLUMN config.maint_message IS '점검 안내 메시지(사용자에게 노출)';
...
-- ============================ 개인정보 접근 로그 ============================
-- event_log는 "무엇을 바꿨나"(등록·수정·삭제)를 남긴다. 조회는 일부러 제외돼 있는데,
-- 개인정보는 **읽은 것 자체가 처리**라 따로 남겨야 한다(누가 언제 누구의 정보를 어느 화면에서 봤는지).
-- 나중에 붙이면 과거 기록을 만들 수 없으므로 빈 화면이어도 지금부터 쌓는다.
--
-- 적재는 자동이다: 개인정보를 복호화하는 SQL(DECRYPT)이 실행되면 요청 단위로 1건 남는다.
-- ⚠ 이 테이블은 자동 삭제하지 않는다 — 접근기록은 법정 최소 보존기간이 있어 지우는 쪽이 위험하다.
--   보관 정책은 운영자가 직접 정한다(파일 GC·메일 이력과 다른 점).
CREATE TABLE privacy_log (
    privacy_log_id INTEGER     GENERATED BY DEFAULT AS IDENTITY,
    member_id    VARCHAR(30),
    ...
    reg_dt       TIMESTAMP    NOT NULL,
    reg_ip       VARCHAR(45),
    CONSTRAINT pk_privacy_log PRIMARY KEY (privacy_log_id)
);
COMMENT ON TABLE  privacy_log IS '개인정보 접근(조회) 로그 - 복호화 조회가 일어난 요청 단위 1건';
COMMENT ON COLUMN privacy_log.privacy_log_id IS '접근 로그 ID';
COMMENT ON COLUMN privacy_log.member_id IS '조회한 사람(취급자)';
COMMENT ON COLUMN privacy_log.request_uri IS '수행 업무 — 요청 경로(어느 화면/API에서 봤는지)';
COMMENT ON COLUMN privacy_log.sql_ids IS '개인정보를 읽은 조회 ID(매퍼 sql_id, 쉼표 구분)';
COMMENT ON COLUMN privacy_log.target_ids IS '처리한 정보주체(회원 ID). 많으면 앞부분만 남기고 생략 표기';
COMMENT ON COLUMN privacy_log.access_cnt IS '조회된 건수 합계';
COMMENT ON COLUMN privacy_log.device_type IS '기기 유형(desktop/mobile/tablet)';
COMMENT ON COLUMN privacy_log.user_agent IS '브라우저 User-Agent 원문';
COMMENT ON COLUMN privacy_log.reg_dt IS '접근 시각';
COMMENT ON COLUMN privacy_log.reg_ip IS '접속지 IP';
-- 목록은 조회자 필터 + 최신순 페이징이다(기록은 계속 쌓이므로 미리 잡아둔다).
CREATE INDEX IF NOT EXISTS ix_privacy_log_member ON privacy_log (member_id, privacy_log_id DESC);

-- ============================ 메일 템플릿 ============================
-- 발송 지점은 코드(template_cd)로 템플릿을 찾는다 — 문구를 고치려고 배포하지 않기 위함.
-- 본문의 {{키}}는 발송 시 값으로 바뀐다(치환 값은 HTML 이스케이프 후 삽입 — 메일 본문 XSS 차단).
CREATE TABLE mail_template (
    mail_template_id INTEGER      GENERATED BY DEFAULT AS IDENTITY,
...
COMMENT ON TABLE  mail_template IS '메일 템플릿(발송 지점이 template_cd로 찾아 쓴다)';
COMMENT ON COLUMN mail_template.mail_template_id IS '메일 템플릿 ID';
COMMENT ON COLUMN mail_template.template_cd IS '템플릿 코드(발송 코드에서 참조. 사용중인 것끼리 중복 불가)';
COMMENT ON COLUMN mail_template.name IS '템플릿 이름(관리 목록 표시용)';
COMMENT ON COLUMN mail_template.subject IS '메일 제목({{키}} 치환 가능)';
COMMENT ON COLUMN mail_template.content IS 'HTML 본문({{키}} 치환 가능)';
COMMENT ON COLUMN mail_template.variables IS '쓸 수 있는 치환 키 안내(운영자용 메모, 동작에는 영향 없음)';
COMMENT ON COLUMN mail_template.use_yn IS '사용여부(Y/N)';
-- 발송 지점이 코드로 1건을 찾으므로 중복되면 어느 쪽이 나갈지 알 수 없다. 사용중(Y)인 것만 유일하게 막는다
-- (안 쓰는 옛 템플릿은 같은 코드로 남겨 둘 수 있어야 한다).
CREATE UNIQUE INDEX IF NOT EXISTS ux_mail_template_cd ON mail_template (template_cd) WHERE use_yn = 'Y';

-- ============================ 메일 발송 이력 ============================
-- append-only. "보냈는데 안 왔다"는 문의를 가리는 유일한 근거라 실패(FAIL)·미발송(SKIP)도 남긴다.
-- ⚠ 수신자 주소는 개인정보라 member.email과 같은 방식(pgcrypto AES, HEX)으로 저장한다.
-- ⚠ 본문에는 인증번호 같은 값이 그대로 남는다 → mail.log.retention-days 지난 행은 스케줄러가 지운다.
CREATE TABLE mail_log (
    mail_log_id INTEGER      GENERATED BY DEFAULT AS IDENTITY,
    template_cd VARCHAR(30),
    ...
    reg_dt      TIMESTAMP    NOT NULL,
    reg_ip      VARCHAR(45),
    CONSTRAINT pk_mail_log PRIMARY KEY (mail_log_id)
);
COMMENT ON TABLE  mail_log IS '메일 발송 이력(성공/실패/미발송)';
COMMENT ON COLUMN mail_log.mail_log_id IS '발송 이력 ID';
COMMENT ON COLUMN mail_log.template_cd IS '사용한 템플릿 코드(템플릿 없이 직접 발송하면 NULL)';
COMMENT ON COLUMN mail_log.to_email IS '수신자 이메일(pgcrypto AES 암호화, HEX)';
COMMENT ON COLUMN mail_log.subject IS '실제 발송된 제목(치환 후)';
COMMENT ON COLUMN mail_log.content IS '실제 발송된 본문(치환 후)';
COMMENT ON COLUMN mail_log.status_cd IS '발송 결과(code MAIL00: SUCCESS/FAIL/SKIP). SKIP=mail.enabled=false라 보내지 않음';
COMMENT ON COLUMN mail_log.error_msg IS '실패 사유 또는 경고(예: 치환되지 않은 키)';
COMMENT ON COLUMN mail_log.reg_id IS '발송을 유발한 사용자(시스템 발송은 NULL)';
-- 이력 목록은 결과 필터 + 최신순 페이징이다(로그는 계속 쌓이므로 미리 잡아둔다).
CREATE INDEX IF NOT EXISTS ix_mail_log_status ON mail_log (status_cd, mail_log_id DESC);
...
-- ============================ 배너(메인 히어로 슬라이드) ============================
-- 메인 첫 화면의 슬라이드를 운영자가 관리한다(예전에는 프론트 코드 상수였다).
--
-- 표시 규칙(프론트가 해석):
--   · title 의 [[...]] 구간은 강조(그라디언트 글자). 예: '혼자 하던 취미,\n이제 [[함께]]'
--   · title·description 의 줄바꿈(\n)은 그대로 살린다 — 문장을 의도한 지점에서 끊기 위함
--   · 버튼 주소가 '#id'면 라우팅 대신 메인 안의 그 섹션으로 스크롤한다
--   ⚠ HTML을 넣지 않는다(저장값을 그대로 렌더하면 XSS 통로가 된다). 마커만 해석한다.
-- 배경 이미지는 file_ref(file_type='BANNER')로 연결. 없으면 CSS 기본 그라디언트가 나온다.
CREATE TABLE banner (
    banner_id   INTEGER      GENERATED BY DEFAULT AS IDENTITY,
    title       VARCHAR(200) NOT NULL,
    description VARCHAR(500),
    btn1_label  VARCHAR(50),
    btn1_url    VARCHAR(255),
    btn2_label  VARCHAR(50),
...
COMMENT ON TABLE  banner IS '메인 히어로 배너(배경 이미지는 file_ref file_type=BANNER 로 연결)';
COMMENT ON COLUMN banner.banner_id IS '배너 ID';
COMMENT ON COLUMN banner.title IS '제목. [[...]] 구간은 강조 표시, \n은 줄바꿈(HTML 아님)';
COMMENT ON COLUMN banner.description IS '설명(줄바꿈 그대로 표시)';
COMMENT ON COLUMN banner.btn1_label IS '주 버튼 문구(비우면 버튼 없음)';
COMMENT ON COLUMN banner.btn1_url IS '주 버튼 이동 경로(/gen/... · #섹션id · http로 시작하는 외부 주소)';
COMMENT ON COLUMN banner.btn2_label IS '보조 버튼 문구';
COMMENT ON COLUMN banner.btn2_url IS '보조 버튼 이동 경로';
COMMENT ON COLUMN banner.start_dt IS '노출 시작일(YYYY-MM-DD, 비우면 제한 없음)';
COMMENT ON COLUMN banner.end_dt IS '노출 종료일(YYYY-MM-DD, 비우면 제한 없음)';
COMMENT ON COLUMN banner.sort_no IS '슬라이드 순서';
COMMENT ON COLUMN banner.use_yn IS '사용여부(Y/N)';
...
-- 약관 동의 이력 — 가입 시 동의한 약관을 회원별로 남긴다.
--   개인정보 수집·이용 동의는 "언제, 어느 버전에 동의했는지"를 증빙해야 하므로 행으로 보관한다.
--   약관 개정 시 policy 행을 새로 등록(기존 행은 use_yn='N')하면, 과거 동의 이력이 그대로 남는다.
CREATE TABLE member_policy (
    ...
    reg_ip    VARCHAR(45)  NOT NULL,
    upd_ip    VARCHAR(45)  NOT NULL,
    CONSTRAINT pk_member_policy PRIMARY KEY (member_id, policy_id)
);
COMMENT ON TABLE  member_policy IS '약관 동의 이력(회원-약관)';
COMMENT ON COLUMN member_policy.member_id IS '회원 ID(member)';
COMMENT ON COLUMN member_policy.policy_id IS '동의한 약관 ID(policy)';
COMMENT ON COLUMN member_policy.reg_dt IS '동의 일시';
COMMENT ON COLUMN member_policy.reg_ip IS '동의 시점 IP(동의 증빙)';
COMMENT ON COLUMN member_policy.use_yn IS '사용여부(Y/N)';
...
COMMENT ON COLUMN file_ref.file_type IS '용도 구분(POST/POST_IMG/POST_EDITOR/POPUP/LOGO/BANNER 등)';
...
-- ============================ 접속 세션 ============================
-- JWT는 무상태라 서버가 "지금 누가 접속 중인지"를 스스로 알 수 없다 → 로그인 시 한 행을 열고,
-- 요청이 올 때마다 last_seen_dt를 갱신(과다 쓰기를 막으려 서비스가 60초 단위로 묶어서 반영)해
-- 접속 현황을 만든다. 종료는 end_dt/end_reason으로 남긴다(행을 지우지 않는다 — 접속 이력).
-- 단일세션(last-wins)이라 한 사용자의 열린 행은 항상 최대 1개다.
CREATE TABLE login_session (
    login_session_id INTEGER     GENERATED BY DEFAULT AS IDENTITY,
    member_id        VARCHAR(30) NOT NULL,
    device_type      VARCHAR(20),
    user_agent       TEXT,
    ...
    reg_ip           VARCHAR(45),
    CONSTRAINT pk_login_session PRIMARY KEY (login_session_id)
);
COMMENT ON TABLE  login_session IS '접속 세션(로그인~종료). event_log와 같은 로그성 테이블이라 use_yn·수정이력 없음';
COMMENT ON COLUMN login_session.login_session_id IS '세션 ID';
COMMENT ON COLUMN login_session.member_id IS '접속자 ID(member)';
COMMENT ON COLUMN login_session.device_type IS '기기 유형(desktop/mobile/tablet)';
COMMENT ON COLUMN login_session.user_agent IS '브라우저 User-Agent 원문';
COMMENT ON COLUMN login_session.login_dt IS '로그인 시각';
COMMENT ON COLUMN login_session.last_seen_dt IS '마지막 활동 시각(세션 만료 판정 기준)';
COMMENT ON COLUMN login_session.end_dt IS '종료 시각. NULL이면 아직 열려 있음';
COMMENT ON COLUMN login_session.end_reason IS '종료 사유 — LOGOUT(본인 로그아웃)/FORCE(관리자 강제종료)/RELOGIN(다른 기기 새 로그인)/PWCHANGE(비밀번호 변경)/WITHDRAW(탈퇴)/SUSPEND(관리자 정지)';
-- 열린 세션 조회·중복 방지용(부분 인덱스라 종료된 행은 색인하지 않는다)
CREATE INDEX ix_login_session_open ON login_session (member_id) WHERE end_dt IS NULL;
CREATE INDEX ix_login_session_member ON login_session (member_id, login_session_id DESC);

-- ============================ 관리자 접속 허용 IP ============================
-- config.acc_ip_yn = 'Y' 일 때만 강제된다. 관리자 계정(회원유형 MEM02)의 요청만 검사하고,
-- 일반 회원·비로그인 사용자는 대상이 아니다(사용자 사이트가 IP로 막히면 안 된다).
-- ⚠ 목록이 비어 있으면 아무도 관리자 기능을 쓸 수 없으므로, 비어 있을 땐 제한을 적용하지 않는다(잠금 방지).
CREATE TABLE access_ip (
    access_ip_id INTEGER      GENERATED BY DEFAULT AS IDENTITY,
    ...
    reg_ip       VARCHAR(45)  NOT NULL,
    upd_ip       VARCHAR(45)  NOT NULL,
    CONSTRAINT pk_access_ip PRIMARY KEY (access_ip_id)
);
COMMENT ON TABLE  access_ip IS '관리자 접속 허용 IP(화이트리스트)';
COMMENT ON COLUMN access_ip.access_ip_id IS '허용 IP ID';
COMMENT ON COLUMN access_ip.ip IS '허용 IP — 정확히 일치(192.168.0.10) 또는 CIDR 대역(192.168.0.0/24)';
COMMENT ON COLUMN access_ip.description IS '설명(사무실·VPN 등 어디인지)';
COMMENT ON COLUMN access_ip.use_yn IS '사용여부(Y/N)';

-- ============================ 금칙어 ============================
-- 게시글·댓글 본문/제목에서 걸러낼 단어. 전 게시판 공통(게시판별 예외가 필요해지면 board_id 컬럼을 추가한다).
CREATE TABLE banword (
    banword_id INTEGER      GENERATED BY DEFAULT AS IDENTITY,
    ...
    reg_ip     VARCHAR(45)  NOT NULL,
    upd_ip     VARCHAR(45)  NOT NULL,
    CONSTRAINT pk_banword PRIMARY KEY (banword_id)
);
COMMENT ON TABLE  banword IS '금칙어(게시글·댓글 등록 시 차단)';
COMMENT ON COLUMN banword.banword_id IS '금칙어 ID';
COMMENT ON COLUMN banword.word IS '금칙어(대소문자 무시 부분일치로 검사)';
COMMENT ON COLUMN banword.use_yn IS '사용여부(Y/N)';
-- 같은 단어를 두 번 등록하지 못하게(활성 행 기준). 대소문자 차이도 중복으로 본다.
CREATE UNIQUE INDEX IF NOT EXISTS ux_banword_word ON banword (LOWER(word)) WHERE use_yn = 'Y';
...
-- ============================ 폼(신청·민원·설문) ============================
-- 신청서·민원접수·설문은 "문항을 정의하고 → 답을 받고 → 모아 본다"가 똑같다.
-- 따로 만들면 세 벌이 되므로 한 엔진으로 두고, 용도는 type_cd로만 구분한다.
-- 사용자 노출은 메뉴 연결유형 MENU05(폼) + conn_id=form_id → /gen/form/{form_id} (게시판·페이지와 같은 방식).
CREATE TABLE form (
    ...
    multi_yn     VARCHAR(1)   NOT NULL DEFAULT 'N',
    done_message TEXT,
    use_yn       VARCHAR(1)   NOT NULL DEFAULT 'Y',
    reg_id       VARCHAR(30)  NOT NULL,
    upd_id       VARCHAR(30)  NOT NULL,
...
COMMENT ON TABLE  form IS '폼 정의(신청·민원·설문 공용)';
COMMENT ON COLUMN form.form_id IS '폼 ID(메뉴 conn_id로 연결)';
COMMENT ON COLUMN form.title IS '폼 제목';
COMMENT ON COLUMN form.description IS '폼 설명(응답 화면 상단 안내)';
COMMENT ON COLUMN form.type_cd IS '용도(code FORM00: FORM01 신청/FORM02 민원/FORM03 설문)';
COMMENT ON COLUMN form.start_dt IS '접수 시작일(YYYY-MM-DD, 비우면 제한 없음)';
COMMENT ON COLUMN form.end_dt IS '접수 종료일(YYYY-MM-DD, 비우면 제한 없음)';
COMMENT ON COLUMN form.login_yn IS '로그인 필요 여부(N이면 비로그인도 제출 가능)';
COMMENT ON COLUMN form.multi_yn IS '같은 사람의 중복 제출 허용(N이면 1인 1회 — 로그인 응답에만 적용)';
COMMENT ON COLUMN form.done_message IS '제출 완료 후 보여줄 안내 문구';
COMMENT ON COLUMN form.use_yn IS '사용여부(Y/N)';

-- 문항. 선택지는 options에 줄바꿈으로 나열한다(별도 테이블을 두면 화면 한 번 저장에 3단 저장이 된다).
CREATE TABLE form_field (
    form_field_id INTEGER      GENERATED BY DEFAULT AS IDENTITY,
    form_id     INTEGER      NOT NULL,
    label       VARCHAR(200) NOT NULL,
    field_cd    VARCHAR(20)  NOT NULL,
    options     TEXT,
    placeholder VARCHAR(200),
    required_yn VARCHAR(1)   NOT NULL DEFAULT 'N',
    privacy_yn  VARCHAR(1)   NOT NULL DEFAULT 'N',
    sort_no     INTEGER,
    use_yn      VARCHAR(1)   NOT NULL DEFAULT 'Y',
    reg_id      VARCHAR(30)  NOT NULL,
    upd_id      VARCHAR(30)  NOT NULL,
...
COMMENT ON TABLE  form_field IS '폼 문항';
COMMENT ON COLUMN form_field.form_field_id IS '문항 ID';
COMMENT ON COLUMN form_field.form_id IS '폼 ID';
COMMENT ON COLUMN form_field.label IS '문항 제목';
COMMENT ON COLUMN form_field.field_cd IS '문항 유형(code FIELD00: 단답/장문/단일선택/다중선택/드롭다운/날짜/숫자/이메일)';
COMMENT ON COLUMN form_field.options IS '선택지(줄바꿈 구분). 선택형 문항에만 사용';
COMMENT ON COLUMN form_field.placeholder IS '입력 안내문(입력형 문항)';
COMMENT ON COLUMN form_field.required_yn IS '필수 응답 여부(Y/N)';
COMMENT ON COLUMN form_field.privacy_yn IS '개인정보 문항 여부 — Y면 답을 암호화해 저장하고 조회가 접근기록에 남는다';
COMMENT ON COLUMN form_field.sort_no IS '문항 순서';
COMMENT ON COLUMN form_field.use_yn IS '사용여부(Y/N)';

-- 응답 1건(제출 1회).
CREATE TABLE form_answer (
    form_answer_id INTEGER     GENERATED BY DEFAULT AS IDENTITY,
    form_id     INTEGER      NOT NULL,
    member_id   VARCHAR(30),
    status_cd   VARCHAR(20)  NOT NULL DEFAULT 'ANSWER01',
    admin_memo  TEXT,
    use_yn      VARCHAR(1)   NOT NULL DEFAULT 'Y',
    reg_id      VARCHAR(30)  NOT NULL,
    upd_id      VARCHAR(30)  NOT NULL,
    ...
    reg_ip      VARCHAR(45)  NOT NULL,
    upd_ip      VARCHAR(45)  NOT NULL,
    CONSTRAINT pk_form_answer PRIMARY KEY (form_answer_id)
);
COMMENT ON TABLE  form_answer IS '폼 응답(제출 1회 = 1행)';
COMMENT ON COLUMN form_answer.form_answer_id IS '응답 ID';
COMMENT ON COLUMN form_answer.form_id IS '폼 ID';
COMMENT ON COLUMN form_answer.member_id IS '제출자(비로그인 제출이면 NULL)';
COMMENT ON COLUMN form_answer.status_cd IS '처리상태(code ANSWER00: 접수/처리중/완료/반려). 설문은 접수 그대로 둔다';
COMMENT ON COLUMN form_answer.admin_memo IS '담당자 메모(응답자에게 보이지 않는다)';
COMMENT ON COLUMN form_answer.use_yn IS '사용여부(Y/N, 삭제 시 N)';

-- 문항별 값. 개인정보 문항(privacy_yn='Y')은 value_enc(pgcrypto AES, HEX)에, 그 외는 value에 넣는다.
-- ⚠ 한 컬럼에 섞어 담지 않는 이유: 섞으면 어느 행이 암호화됐는지 알 수 없어 조회 SQL이 항상
--   복호화를 걸어야 하고, 그러면 개인정보가 하나도 없는 설문을 봐도 접근기록에 남아 기록이 무의미해진다.
CREATE TABLE form_answer_value (
    form_answer_id INTEGER   NOT NULL,
    form_field_id  INTEGER   NOT NULL,
    value      TEXT,
    value_enc  TEXT,
    CONSTRAINT pk_form_answer_value PRIMARY KEY (form_answer_id, form_field_id)
);
COMMENT ON TABLE  form_answer_value IS '폼 응답의 문항별 값';
COMMENT ON COLUMN form_answer_value.form_answer_id IS '응답 ID';
COMMENT ON COLUMN form_answer_value.form_field_id IS '문항 ID';
COMMENT ON COLUMN form_answer_value.value IS '답(일반 문항). 다중선택은 줄바꿈으로 이어 붙인다';
COMMENT ON COLUMN form_answer_value.value_enc IS '답(개인정보 문항, pgcrypto AES 암호화 HEX)';
...
CREATE INDEX IF NOT EXISTS ix_form_field_form  ON form_field (form_id, sort_no);          -- 폼 문항 순서대로
CREATE INDEX IF NOT EXISTS ix_form_answer_form ON form_answer (form_id, form_answer_id DESC); -- 폼별 응답 최신순
-- 1인 1회 제한 검사(폼+제출자). 제출 때마다 도는 조회라 미리 잡아둔다
CREATE INDEX IF NOT EXISTS ix_form_answer_member ON form_answer (form_id, member_id);