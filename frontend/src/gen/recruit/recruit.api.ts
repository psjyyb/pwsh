...
export interface Recruit {
  ...
  areaName?: string // 지역명(표시용)
  region?: string // 상세 지역(자유입력: 구/동/장소)
  placeName?: string // 만날 장소명(지도 선택) — 목록/상세
  ...
  statusCd?: string
  viewCnt?: string
  hobbyName?: string // 취미명
  statusName?: string // 모집상태명
  regId?: string // 주최자 로그인 ID — 관리자 화면 전용(사용자(GEN) 응답에는 내려오지 않음)
  regName?: string // 주최자 닉네임
...
export interface RecruitApply {
  rowId?: string
  recruitId?: string
  memberId?: string // 신청자 로그인 ID — 주최자·관리자용 신청자 목록에서만 내려옴(내 신청 내역엔 없음)
  applyCd?: string
  applyMemo?: string
  applyName?: string
  waitNo?: string     // 대기 순번(대기 상태일 때만, 신청순 1부터)
  attendCd?: string   // 참석 결과(ATTEND00). 미기록이면 없음
  attendName?: string   // 참석 결과명
  memberHandle?: string // 신청자 공개 식별자(프로필 링크)
...
export const applyApi = {
  ...
  changeStatus: (rowId: string, applyCd: string) =>
    apiPost<void>('/adm/recruitApply/updateRecruitApply.do', { rowId, applyCd }),
