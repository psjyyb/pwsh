...
import { BOARD_LIST_URL, boardApi } from './board.api'
import type { Board } from './board.api'

/** 게시판관리 — 게시판 정의(설정) 분할 마스터-디테일 CRUD. */
export default function BoardListPage() {
  const navigate = useNavigate()
  const { rows, total, loading, page, pageSize, reload, search, changePage } = useList<Board>(BOARD_LIST_URL)
  const { form, mode, selectedKey, openNew, openRow, save, remove } = useSplitForm<Board>(boardApi, reload)
  const isEdit = mode === 'edit'
  const fileYnWatch = Form.useWatch('fileYn', form) // 첨부 사용여부 → 개수/용량 입력 노출 제어
  const cdWatch = Form.useWatch('typeCd', form)
  const isFaqType = cdWatch === 'BOARD02' // FAQ는 첨부 개념 자체가 없음
  ...
  const columns: TableColumnsType<Board> = [
    ...
    { title: '유형', width: 120, render: (_, r) => r.boardCdName ?? r.typeCd },
    { title: '사용', dataIndex: 'useYn', width: 60 },
    {
      // 취미 게시판은 메뉴로 등록하지 않으므로(취미 등록 시 자동 생성) 여기가 글 관리 진입점이다.
      title: '글',
      width: 80,
      render: (_, r) => (
        <a onClick={(e) => { e.stopPropagation(); navigate(`/adm/post/${r.rowId}`) }}>글 관리</a>
      ),
    },
  ]
    ...
    >
      {mode === 'none' ? (
        ...
        <Form
          form={form}
          layout="vertical"
          initialValues={{ listCnt: '10', fileYn: 'N', fileCntLimit: '5', fileSizeLimitMb: '10', noticeYn: 'N', newCnt: '0' }}
        >
          <Form.Item name="boardName" label="게시판명" rules={[{ required: true, message: '게시판명을 입력하세요.' }]}>
            <Input />
          </Form.Item>
          <Form.Item name="typeCd" label="게시판 유형" rules={[{ required: true, message: '유형을 선택하세요.' }]}>
            <CodeSelect pCodeId="BOARD00" placeholder="유형 선택" />
          </Form.Item>
          ...
          {!isFaqType && (
            <div>
              <Space size={8} wrap align="baseline">
                ...
                {fileYnWatch === 'Y' && <Form.Item name="fileCntLimit" label="첨부 개수"><NumberInput /></Form.Item>}
                {fileYnWatch === 'Y' && <Form.Item name="fileSizeLimitMb" label="첨부 용량(MB)"><NumberInput /></Form.Item>}
              </Space>
            </div>
          )}
        </Form>
