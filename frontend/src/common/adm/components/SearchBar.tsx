...
export type SearchField =
  ...
  | {
      type: 'keyword'
      name?: string // 검색어 파라미터명(기본 searchKeyword)
      condName?: string // 검색조건 파라미터명(기본 searchCondition)
...
export default function SearchBar({ fields, onSearch, onCreate, createText = '등록', leftExtra }: Props) {
  ...
  const buildDefaults = () => {
    ...
    fields.forEach((f) => {
      if (f.type === 'keyword') d[f.condName ?? 'searchCondition'] = f.conditions[0]?.value ?? ''
    })
    return d
  }
  ...
  const reset = () => {
    ...
    fields.forEach((f) => {
      if (f.type === 'keyword') cleared[f.name ?? 'searchKeyword'] = ''
      else cleared[f.name] = ''
    })
    setValues(cleared)
    onSearch(cleared)
  ...
  const renderField = (f: SearchField) => {
    if (f.type === 'keyword') {
      const kwName = f.name ?? 'searchKeyword'
      const condName = f.condName ?? 'searchCondition'
      return (
