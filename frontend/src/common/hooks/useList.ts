 ...
 *
 *   const { rows, total, loading, page, size, search, changePage, reload } =
...
export function useList<T>(url: string, initialParams: Record<string, unknown> = {}) {
  ...
  const load = async (p: number, sz: number, prm: Record<string, unknown>) => {
    setLoading(true)
    try {
      const res = await apiPost<ListResult<T>>(url, { pageIndex: p, size: sz, ...prm })
      setRows(res.list)
      setTotal(res.totCnt)
      setPage(p)
      setSize(sz)
  ...
  return {
    ...
    reload: () => load(page, size, params),
    /** 검색조건 갱신 후 1페이지부터 조회 */
    search: (prm: Record<string, unknown>) => load(1, size, { ...params, ...prm }),
    /** 페이지/사이즈 변경 */
