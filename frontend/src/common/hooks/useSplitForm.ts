...
interface CrudApiLike<T> {
  ...
  remove: (dbKey: string) => Promise<unknown>
}
 ...
 * 폼 컴포넌트 1개를 재사용: mode로 등록/수정 구분, 수정 시 dbKey(자기 PK)로 갱신.
 *   const { form, mode, selectedKey, openNew, openRow, save, remove } = useSplitForm(codeApi, reload)
 * - openRow(dbKey): 해당 행 상세를 폼에 바인딩(수정 모드) — view.do로 전체 필드 로드
 * - openNew(): 빈 폼(등록 모드)
...
export function useSplitForm<T extends Record<string, any>>(api: CrudApiLike<T>, onChanged: () => void) {
  ...
  const openRow = async (dbKey: string) => {
    try {
      const data = await api.view(dbKey)
      form.resetFields()
  ...
  const save = async () => {
    ...
    try {
      if (mode === 'edit') {
        await api.update({ ...values, dbKey: selectedKey })
      } else {
