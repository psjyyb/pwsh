...
CREATE TABLE t_notification (
    ...
    noti_type  VARCHAR(20)  NOT NULL,   -- APPLY(신청)/ACCEPT(수락)/REJECT(거절)/COMMENT(댓글)/MESSAGE(쪽지)/REVIEW(후기)/REMIND(모임 하루 전)/NEWRECRUIT(담은 취미 새 모집)
...
COMMENT ON COLUMN t_notification.noti_type IS '유형 APPLY/ACCEPT/REJECT/COMMENT/MESSAGE/REVIEW/REMIND';
